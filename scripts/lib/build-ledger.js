...
// module's job, and it does it from declared metadata, never from ordering.
...
/**
 * Resolve domain links for a unit from an explicit mapping.
 *
 * `mapping` is caller-supplied metadata (annotations, collection variables,
 * a mapping file) -- never inferred from position or failure ordering (B6).
 * An unmatched unit keeps null links and states why.
 *
 * @param {object[]} units
 * @param {object} mapping  { [unit_id]: {test_case_id?, api_test_case_id?, ...} }
...
export function resolveDomainLinks(units, mapping = {}) {
  ...
  const resolved = units.map((u) => {
    ...
    if (!m) {
      unmapped.push(u.unit_id);
      return u; // keeps the adapter's null links + reason
    }

    // An entry claiming two different cases for one unit is ambiguous, not a
    // coin flip.
    if (Array.isArray(m)) {
    ...
    const stillNull = Object.entries(links).some(
      ([k, v]) => k !== 'unresolved_reason' && v === null
    );
    if (!stillNull) delete links.unresolved_reason;
    else if (any) {
      links.unresolved_reason =
        'Partially resolved: the listed IDs are proven from metadata; the remaining links have no declared mapping.';
    }
