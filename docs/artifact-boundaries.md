...
| `evidence/`                      | Human / team (via `scripts/benchmark-capture.js`)          |   IP5   | Benchmark measurements (`benchmark.jsonl`, one schema-valid record per story×arm) behind `docs/evidence.md`. Appended by the capture script; never edited by an agent. See `docs/benchmark-protocol.md`.                                                                                                                                                                                                                                                                |
...
| `analysis/execution-ledger.json` | **Execution adapters** (via the shared normalizer)         |    3    | The canonical cross-runner counting model for a run (`schemas/execution-ledger.schema.json`). **Produced by** the execution adapters only; **consumed by** the Failure Classifier, the Reporter, and the TestLink execution sync — none of them write it. Contract and readers exist as of task group 2.2a; the adapters that WRITE it arrive in Phase 3, so the file is absent until then. Sanitized: never raw request/response bodies, headers, or injected secrets. |
